...
class Server {
    ...
    public void run() throws IOException, SQLException {
        ...
        Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/main","root","2903");
        ...
        while (true) {
            ...
            stmt.executeUpdate("INSERT INTO log(name, time) VALUES ('" + nickname + "', '" + timeStamp + "');");
            System.out.println("New Client: \"" + nickname + "\"\n\t     Host:" + client.getInetAddress().getHostAddress());
            RandomAccessFile writer;
            try {
                writer = new RandomAccessFile("logs.txt", "rw");
                writer.seek(writer.length());
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
            writer.writeUTF("\n"+nickname+" joined at "+timeStamp);
            // create new User
        ...
        while (sc.hasNextLine()) {
            ...
            if (message.charAt(0) == '@') {
                if (message.contains(" ")) {
                    ...
                    try {
                        writer.writeUTF("\nprivate msg to "+userPrivate+" by "+user.getNickname()+": "+message);
                    } catch (IOException e) {
                        throw new RuntimeException(e);
                    }
                ...
                try {
                    writer.writeUTF("\ncolor change by "+user.getNickname()+" to "+message);
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
